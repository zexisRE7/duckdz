...
// ── Vvector3 — used with Transform_INTERNAL_SetPosition ──────────────────────
struct Vvector3 {
    float x, y, z;
    Vvector3(float x = 0, float y = 0, float z = 0) : x(x), y(y), z(z) {}
};

// ── Dictionary<K,V> — Unity IL2CPP dictionary wrapper ────────────────────────
...
public:
    ...
    TValue *getValues() {
        return reinterpret_cast<TValue *>(
            reinterpret_cast<uint8_t *>(entries) + offsetof(Entry, value));
    }
};
