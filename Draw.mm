...
// ── ZXToggle — custom checkmark-style toggle ──────────────────────────────────
...
- (instancetype)initWithFrame:(CGRect)f {
    ...
    if (self) { [self addTarget:self action:@selector(_tap) forControlEvents:UIControlEventTouchUpInside]; [self _draw]; }
    return self;
}
...
- (void)_draw {
    ...
    if (_on) {
        ...
        self.backgroundColor = [UIColor colorWithRed:0.22 green:0.25 blue:0.31 alpha:1.0];
    }
}
@end
...
- (void)buildHeader {
    ...
    [moonBtn setTitleColor:[UIColor colorWithRed:0.23 green:0.51 blue:0.96 alpha:1.0] forState:UIControlStateNormal];
    moonBtn.titleLabel.font = [UIFont systemFontOfSize:14];
    ...
    [saveBtn setTitleColor:[UIColor colorWithWhite:0.7 alpha:1.0] forState:UIControlStateNormal];
...
- (void)buildSidebar {
    ...
    for (int i = 0; i < (int)icons.count; i++) {
        ...
        if (i == 0) {
            btn.backgroundColor = [UIColor colorWithRed:0.23 green:0.51 blue:0.96 alpha:0.25];
            btn.layer.borderColor = [UIColor colorWithRed:0.23 green:0.51 blue:0.96 alpha:0.6].CGColor;
...
- (void)tabTapped:(UIButton *)btn {
    ...
    btn.backgroundColor = [UIColor colorWithRed:0.23 green:0.51 blue:0.96 alpha:0.25];
    btn.layer.borderColor = [blue colorWithAlphaComponent:0.6].CGColor;
    btn.layer.borderWidth = 1.5;

    NSArray *names = @[@"⊕  AIMBOT", @"👁  ESP", @"🎮  MISC", @"🔧  WEAPONS", @"👤  PROFILE"];
...
- (ZXToggle *)addRow:(NSString *)label
    ...
    CGFloat toggleSz = 32;
    UILabel *lbl = [[UILabel alloc] initWithFrame:CGRectMake(14, 0, w - toggleSz - 24, kPillHeight)];
    lbl.text = label;
    ...
    if (sub) {
        UILabel *subLbl = [[UILabel alloc] initWithFrame:
            CGRectMake(14, kPillHeight - 4, w - toggleSz - 24, 22)];
        subLbl.text = sub;
        subLbl.textColor = [UIColor colorWithWhite:0.45 alpha:1.0];
        subLbl.font = [UIFont systemFontOfSize:11];
        [row addSubview:subLbl];
    }

    ZXToggle *tog = [[ZXToggle alloc] initWithFrame:
        CGRectMake(w - toggleSz - 10, (kPillHeight - toggleSz) / 2, toggleSz, toggleSz)];
    tog.on = on;
...
- (void)buildProfileTab:(CGFloat *)y width:(CGFloat)w {
    ...
    NSArray *vals = @[@"0", [fmt stringFromDate:now], device, iosVer, name, @"1.0.0", @"Not Available"];
    NSArray *valColors = @[
    ...
    for (NSUInteger i = 0; i < keys.count; i++) {
        UIView *row = [[UIView alloc] initWithFrame:CGRectMake(kContentPadding, *y, w, 38)];
        row.backgroundColor = [UIColor colorWithRed:0.10 green:0.15 blue:0.22 alpha:1.0];
        row.layer.cornerRadius = 10;
        [self.contentContainer addSubview:row];

        UILabel *kLbl = [[UILabel alloc] initWithFrame:CGRectMake(12, 0, w * 0.38f, 38)];
        kLbl.text = keys[i];
        kLbl.textColor = [UIColor colorWithWhite:0.6 alpha:1.0];
        kLbl.font = [UIFont systemFontOfSize:13];
        [row addSubview:kLbl];

        UILabel *vLbl = [[UILabel alloc] initWithFrame:CGRectMake(w * 0.42f, 0, w * 0.55f, 38)];
        vLbl.text = vals[i];
        vLbl.textColor = valColors[i];
        vLbl.font = [UIFont boldSystemFontOfSize:13];
        [row addSubview:vLbl];
...
- (void)startParticles {
    NSTimer *t = [NSTimer scheduledTimerWithTimeInterval:1.2
                                                  target:self
...
- (void)spawnParticle {
    ...
    CGFloat sz = 4 + arc4random_uniform(5);
    CGFloat startX = (arc4random_uniform((uint32_t)pw));
    UIView *dot = [[UIView alloc] initWithFrame:CGRectMake(startX,
        floatingPanel.bounds.size.height + sz, sz, sz)];
    dot.backgroundColor = [UIColor colorWithRed:0.23 green:0.51 blue:0.96 alpha:0.5];
    dot.layer.cornerRadius = sz / 2;
    [floatingPanel insertSubview:dot atIndex:0];

    CGFloat dur = 3.0 + (arc4random_uniform(30) / 10.0);
    [UIView animateWithDuration:dur delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
        dot.frame = CGRectMake(startX + (arc4random_uniform(60)) - 30,
                               -sz - 10, sz, sz);
        dot.alpha = 0;
