...
template <uintptr_t Const> struct vxCplConstantify {
    enum { Value = Const };
};
...
constexpr uintptr_t vxCplHashPart1(char Ch, uintptr_t Hash) {
    return (vxCplHashPart2(Ch, Hash) & 0x0FFFFFFF);
}
...
// ✅ Return numeric values for patching logic
#define ENCRYPTOFFSET(Str) strtoull(ENCRYPT(Str), NULL, 0)
#define ENCRYPTHEX(Str)    strtoull(ENCRYPT(Str), NULL, 0)
#define oxo(Str)           strtoull(ENCRYPT(Str), NULL, 0)

// ✅ Optional: return raw decrypted string if needed (for printf/logging)
#define ENCRYPTHEX_STR(Str) ENCRYPT(Str)
