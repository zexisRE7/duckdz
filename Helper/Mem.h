...
static bool getType(unsigned int data) {
...
static MemoryFileInfo getBaseInfo() {
...
static MemoryFileInfo getMemoryFileInfo(const std::string& fileName) {
    MemoryFileInfo _info;
...
static uintptr_t getAbsoluteAddress(const char *fileName, uintptr_t address) {
    MemoryFileInfo info;
    if (fileName)
...
static uint64_t getRealOffset(uint64_t offset){
    return getAbsoluteAddress(NULL, offset);
}

static uint64_t getRealOffsetNULL(uint64_t offset){
    return getAbsoluteAddress(NULL, offset);
}
...
static bool vm_unity(long long offset, unsigned int data) {
...
static bool vm(long long offset, unsigned int data) {
    //Change binary name here if it not UnityFramework
...
static bool vm_anogs(long long offset, unsigned int data) {
    //Change binary name here if it not UnityFramework
